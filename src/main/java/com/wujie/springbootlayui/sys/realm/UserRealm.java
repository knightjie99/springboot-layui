package com.wujie.springbootlayui.sys.realm;
...
public class UserRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        String condition = authenticationToken.getPrincipal().toString();
        if(StringUtil.isPhone(condition)) {
        	queryWrapper.eq("phone",condition);
        }else {
        	queryWrapper.eq("login_name",condition);
        }
        //通过用户名从数据库中查询出该用户
        ...
        if (null!=user){
            ...
            //根据用户ID查询角色ID
            List<Integer>  rid = roleService.queryUserRoleIdsByUid(userId);
            //根据角色ID查询出权限ID
