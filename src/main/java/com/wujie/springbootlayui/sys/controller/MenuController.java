...
import com.wujie.springbootlayui.sys.vo.PermissionVo;
...
public class MenuController {
    ...
    public DataGridView loadIndexLeftMenuJson(PermissionVo permissionVo, HttpServletRequest request){
        ...
        System.out.println("menu lang:"+lang);
        ...
        System.out.println("菜单接口lang: "+user.getLoginLang());
...
//            //1.根据用户ID查询角色
            List<Integer> allRid = roleService.queryUserRoleIdsByUid(userId);
            List<Integer> rid = new ArrayList<>();

            if(allRid.contains(activerUser.getLoginRole())){
                rid.add(activerUser.getLoginRole());
            }
            ...
            //根据角色ID查询菜单ID和权限ID
            List<Integer> permissionIds = roleService.queryRolePermissionIdsByRid(rid.toArray(new Integer[rid.size()]));
            //将菜单ID和权限ID放入Set中去重
