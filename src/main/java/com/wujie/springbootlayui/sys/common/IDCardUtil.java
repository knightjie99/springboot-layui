...
public class IDCardUtil {
    ...
    public  ResultObj checkDB(String str,Integer belongId){

        QueryWrapper<TempVisitor> visitorQueryWrapper = new QueryWrapper<>();
        visitorQueryWrapper.eq("belong_id", belongId);
        visitorQueryWrapper.eq("id_num", str);
        if (visitorService.list(visitorQueryWrapper).size() > 0) {
            return new ResultObj(500, "Idnum_usedBy_visitor");
        }

        QueryWrapper<VipVisitor> vipQueryWrapper = new QueryWrapper<>();
        vipQueryWrapper.eq("belong_id", belongId);
        vipQueryWrapper.eq("id_num", str);
        if (vipVisitorService.list(vipQueryWrapper).size() > 0) {
            return new ResultObj(500, "Idnum_usedBy_vip");
        }

        QueryWrapper<BlackVisitor> blackListQueryWrapper = new QueryWrapper<>();
        blackListQueryWrapper.eq("belong_id", belongId);
        blackListQueryWrapper.eq("id_num",str);
        if (blackListService.list(blackListQueryWrapper).size() > 0) {
            return new ResultObj(500, "Idnum_usedBy_blackList");
        }

        else return new ResultObj(200,"");
